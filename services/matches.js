...
async function getUserMatches(userId, type = "all", matchId = null) {
  try {
    // Create isolated scope for each function call to prevent race conditions
    const functionScope = {
      playersByTeamId: {},
      teamTotals: {},
      ...
      switch (type.toLowerCase()) {
        ...
        case "live":
          return [
            ...
            "Stump Day 3",
            "Stump Day 4",
            "Innings Break", // Include break statuses
            "Tea Break", // Include break statuses
            "Lunch", // Include break statuses
            "Dinner",
            "Live",
            "Int.",
          ];
    ...
    // Debug logging with unique request ID
    const requestId = `${userId}_${Date.now()}_${Math.random()
      .toString(36)
      .substr(2, 9)}`;

    // Get seasonId mapping for each match
    const tournaments = await db("tournaments")
      .select("id", "metadata")
      .whereIn(
        "id",
    ...
    // Get ONLY user's contest entries
    const contestEntries = await db("fantasy_games as fg")
      .join("contests as c", "fg.contest_id", "c.id")
      .leftJoin("leaderboard as lb", "lb.fantasyGameId", "fg.id")
      .leftJoin("fantasy_teams as ft", "fg.fantasy_team_id", "ft.id")
    ...
    // Get ALL contest entries for leaderboard calculation
    const allContestEntries = await db("fantasy_games as fg")
      .join("contests as c", "fg.contest_id", "c.id")
      .leftJoin("leaderboard as lb", "lb.fantasyGameId", "fg.id")
      .join("fantasy_teams as ft", "fg.fantasy_team_id", "ft.id")
    ...
    if (combinedTeamIds.length > 0) {
      ...
      const caseExpr = whenClauses.length
        ? `pt.season_id = (CASE ${whenClauses.join(" ")} ELSE pt.season_id END)`
        : "1=1";
      ...
      functionScope.playersByTeamId = teamPlayersRows.reduce((acc, row) => {
        const dedupKey = `${row.fantasy_team_id}:${row.player_id}`;
        if (seenKeys.has(dedupKey)) return acc;
        seenKeys.add(dedupKey);
        if (!acc[row.fantasy_team_id]) acc[row.fantasy_team_id] = [];
      ...
      // Compute live team totals
      Object.entries(functionScope.playersByTeamId).forEach(
        ([teamId, players]) => {
          let total = 0;
          for (const p of players) {
            let mult = 1;
          ...
          functionScope.teamTotals[teamId] = Number(total.toFixed(2));
        }
      );
    }
    ...
    const matchesWithNotification = matches.map((match) => {
      ...
      if (["Finished", "Completed"].includes(match.status)) {
        ...
        result_note = metadata?.note || scorecard?.note || toss;
      ...
      return {
                ...
                const leaderboardWithCalculatedPoints = allEntriesInContest
                  ...
                  .map((c, index) => {
                    ...
                    if (
                      ...
                      functionScope.playersByTeamId[c.fantasy_team_id]
                    ) {
                      playersArr = functionScope.playersByTeamId[
                        c.fantasy_team_id
                      ].map((player) => ({
                        ...player,
                      }));
        ...
        teams: shouldIncludeContestTeams
          ? Array.from(usedTeamsByMatchId.get(match.id) || []).map(
              ([_, team]) => {
                const players = functionScope.playersByTeamId[team.id] || [];
                ...
                const totalPoints = officialPointsEntry
                  ...
                  : functionScope.teamTotals[team.id] || 0;
    ...
    console.error(`[ERROR] getUserMatches failed: ${error.message}`);
    return [];
  }
}
